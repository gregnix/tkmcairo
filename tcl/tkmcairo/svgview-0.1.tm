...
#     $sv destroy
#
# Requirements: tkmcairo::surface, tclmcairo (+ optional lunasvg, tdom)
#
...
proc ::tkmcairo::svgview::_detectRenderer {pref} {
    ...
    set hasLuna 0
    if {![catch {$tmp svg_size_luna /dev/null} err]} {
        set hasLuna 1
    } else {
        # Fehler ist ok — wichtig ist ob der Befehl existiert
        if {[string match "*no such file*" $err] ||
            [string match "*cannot load*" $err]} {
            set hasLuna 1
        }
    }
    $tmp destroy
    if {$hasLuna} { return luna }
    # Fallback: svg2cairo
...
proc ::tkmcairo::svgview::_load {w file} {
    ...
    if {$r eq "luna"} {
        set tmp [tclmcairo::new 1 1]
        catch {lassign [$tmp svg_size_luna $file] sw sh}
        $tmp destroy
    }
    if {($sw == 0 || $sh == 0)} {
...
proc ::tkmcairo::svgview::_draw {plotw ctx pw ph} {
    ...
    switch $r {
        luna {
            if {[catch {
                $ctx svg_file_luna $file $x $y -width $dw -height $dh
            } err]} {
                # Fallback
                if {![catch {package require svg2cairo}]} {
